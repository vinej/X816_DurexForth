...
3. **The audio note/patch API - SURVEYED, NOT YET PORTED.** The user
   supplied the X16 ROM source at `C:/Users/jyv/Downloads/x16-rom-master`
   and asked for it to land in **X816_Library FIRST**, then be bound
   here. `X816_Library/src_acme/audio/ym.asm` already says this is "a
   project for the day someone wants BASIC-style FMNOTE playing".
   Findings from reading `x16-rom-master/audio/` (~5,400 lines of ca65):
   - `audio_luts.s` is GENERATED by `create_luts.py`, so the tables can
     be re-emitted as ACME `!byte` mechanically - do not hand-convert.
     Six core tables of 128 bytes: `midi2psg_l/h`, `midi2ymkc`,
     `ymkc2midi`, `midi2bas`, `bas2midi`.
   - `kfdelta0_l..6_l` are 12 ZERO bytes each and deliberately OVERLAP
     into the real 128-byte `kfdelta7_l`. That is a compact trick, not
     a bug: low KF bits contribute nothing. Fine pitch only.
   - `notecon_midi2psg` SHORT-CIRCUITS when KF = 0, so plain note
     playing needs `midi2psg` alone and none of the kfdelta machinery.
   - The X816 port is SIMPLER than the original: the ROM juggles
     `ram_bank` around its temp vars, and this machine is flat.
   - **A real bug to fix on the way in**: `notecon_midi2bas` indexes
     `bas2midi` instead of `midi2bas`, so it answers the wrong note
     (MIDI 60 gives 59, not 65). `X816_Library/src_acme/audio/rom.asm`
     already documented it against r49; the port must not copy it.
   Suggested order: tables + noteconvert, then the PSG note API
   (`psg.s`, 639 lines), then FM (`fm.s` 990 + `fmpatches.s` 1189 +
   `fmpatchtables.s` 427), then `playstring.s` (961) last.
