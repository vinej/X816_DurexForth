...
python build/parencheck.py bench/*.fs || exit 1
./build.sh || exit 1
...
SRC = [("forth", n) for n in ["base", "asm", "wordlist", "labels", "doloop",
      ...
      [("forth/mod", "float")] + \
      [("bench", n) for n in ["bench", "bm1", "bm2", "bm3", "bm4", "bm5",
                              "bm6", "bm7", "bm8", "ahl"]]
for d, n in SRC:
...
WHAT = {
    ...
    "BM8": "K^2, LN, SIN via FLOAT (IEEE, the SuperBasic engine)",
...
if acc:
    print()
    print("Ahl's accuracy, 0 being perfect (IEEE-754 singles through")
    print("fpengine.bin -- the same bits SuperBasic computes):")
